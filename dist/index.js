define("pkg/omelete-db/0.0.1/index", [], function() { return /******/ (function(modules) { // webpackBootstrap
/******/ 	// The module cache
...
var Defer = function () {
  ...
  _createClass(Defer, [{
    ...
    key: 'fire',
    value: function fire(cbs, res, flag) {
    ...
    key: 'resolve',
    value: function resolve(res, flag) {
      this.fire(this.successCallback, res, flag).fire(this.finalCallback, res, flag);
    }
  }, {
    key: 'reject',
    value: function reject(res, flag) {
      if (window._ENV_ === 'dev') {
        throw new Error('You need use fail method to get the error: ' + JSON.stringify(res));
      }
      this.fire(this.failCallback, res, flag).fire(this.finalCallback, res, flag);
    }
  }]);
...
var _defer = __webpack_require__(0);

var _defer2 = _interopRequireDefault(_defer);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var OmeleteDB = _core2.default;
OmeleteDB.Defer = _defer2.default;
...
var _tapable = __webpack_require__(4);
...
var _catch = __webpack_require__(7);
...
var Core = function (_Tapable) {
  _inherits(Core, _Tapable);
  ...
  function Core(options) {
    ...
    _this.options = _jquery2.default.extend({}, options || {}, _this.constructor.defaultOptions);
    // 注册缓存插件
    (0, _cache2.default)(_this);
    return _this;
  }
  // ?? 这个defer 哪里传入？
  ...
  _createClass(Core, [{
    key: 'ajax',
    value: function ajax(options, defer) {
      var _this2 = this;

      defer = defer || new _defer2.default();
      // 合并所有 options
      var opts = this.applyPluginsWaterfall('options', _jquery2.default.extend({}, this.options, options));
      // console.log('opts', opts)
      // 注册 endpoint 插件 如果有一个插件可以执行 则立即执行
      var endpoint = this.applyPluginsBailResult('endpoint', opts, defer)(opts).then((0, _catch2.default)(function (success) {
        console.log('success', success);
        // 注册 judge 插件， 返回值为 Boolean??
        var flag = _this2.applyPluginsBailResult('judge', success, opts);
        if (flag === false) {
          // 注册 fail 插件
          success = _this2.applyPluginsWaterfall('error', success, opts);
          defer.reject(success);
        }
        // resolve plugin
        _this2.applyPlugins('resolve', success, opts, defer);
      }, defer, opts), (0, _catch2.default)(function (error) {
        console.log('error', error);
        error = _this2.applyPluginsWaterfall('error', error, opts);
        defer.reject(error);
      }), defer, opts);
...
function parseData(data) {
  if (!data) return '';
  return Object.keys(data).filter(function (v) {
    return v.charAt(0) !== '_';
  }).sort().reduce(function (res, cur) {
...
function parseKey(options) {
  var key = options.__key__ || options.url + '?' + parseData(options.data);
  options.__key__ = key;
  return key;
}
...
function hasCache(options) {
  ...
  var key = _storage2.default.has(parseKey(options), options.level);
  // 如果 localStorage 中存在 key， 且 有 lazy状态和refresh 更新时间
  if (key && options.lazy && options.refresh) {
    // 取出 缓存中 其 储存的时间点
    var lastTime = +key.slice(key.length - 13);
...
function fromCache(defer, options) {
  var res = _storage2.default.get(options.hasCache);
  // 从 cache 中 获取的数据 设置 __flag__: 'CACHE'
  var p = new Promise(function (resolve, reject) {
    setTimeout(function () {
      if (res) {
        resolve(_jquery2.default.extend({
          __flag__: 'CACHE'
        }, res));
      }
    }, 0);
  ...
  if (options.refresh && !options.lazy) {
    var _hasCache = options.hasCache;
    // 取出 缓存中 其 储存的时间点
    var lastTime = +_hasCache.slice(_hasCache.length - 13);
  ...
  this.ajax(_jquery2.default.extend({}, options, {
    nocache: 'again',
    hasCache: false
  }), defer);
...
module.exports = function (instance) {
  // localstorage
  if (_storage2.default.compat()) {
    _storage2.default.init();
    // 初始化了 localstorage

    instance.plugin('options', function (options) {
    ...
    instance.plugin('endpoint', function (options, defer) {
      if (options.hasCache) {
        // 如果判断有缓存，则做获取操作
    ...
    instance.plugin('resolve', function (res, opts, defer) {
      ...
      if ((!opts.nocache || opts.nocache === 'again') && !res.__flag__) {
        ...
        _storage2.default.save(parseKey(opts), res, opts.level);
      }
...
function getTarget(key, level) {
  return level > 3 ? 'omelete' + level + '\u2702$_SYSTEM_$\u2702' + key + '\u2702' : 'omelete' + level + '\u2702' + mid + '\u2702' + key + '\u2702';
}
...
function has(key) {
  ...
  var target = getTarget(key, level);
...
function save2Local(key, value, level, removeLevel) {
  ...
  setTimeout(function () {
    ...
    var k = '' + getTarget(key, level) + +new Date();
    // 存爆处理
    try {
...
function removeData(level) {
  ...
  if (level > 4) {
    ...
    for (var i = 0, l = keys.length; i < l; i++) {
      var match = keys[i].match(reg);
      if (match && match[1] !== mid // 不属于该用户
      ) {
          removeList.push(keys[i]);
        }
    }
  } else {
    var target = 'omelete' + level + '\u2702';
    for (var _i = 0, len = keys.length; _i < len; _i++) {
      if (keys[_i].indexOf(target) === 0) {
        // 找到这些等级的数据
        removeList.push(keys[_i]);
      }
    }
  }
  removeKeys(removeList);
...
var formatName = function formatName(opts) {
  var name = 'DB(' + opts.url + ')';
  if (_typeof(opts._inject_) === 'object') {
    name = 'Component(' + opts._inject_.name + '@' + opts._inject_.version + ').' + name;
  }
  return name;
};

module.exports = function (fn, defer, opts) {
  return function (arg) {
    // 捕获 请求完成时 处理的错误
    try {
      fn(arg);
