...
第 11 个测试用例
> 如果同时存在 lazy 和 refresh。
> 1.如果存在lazy状态，没有超过 refresh 时间，直接从缓存中取数据，执行操作。并发起一次真实请求，更新缓存中的数据。
> 2.如果存在lazy状态，超过了refresh时间，重新发起请求，并更新缓存中的数据，执行操作。
> 处理：如果超过了数据缓存时间，则直接返回false，表示没有缓存数据。
> ![Alt text](./11-1.png)
> 验证；
> 1.已有缓存，发起get，lazy状态，在refresh更新时间内（数据未失效）
> ![Alt text](./11-2.png)
> ![Alt text](./11-3.png)
> 蓝色箭头，代表数据未过期，从缓存中取到数据，并发起一次请求，并缓存，不做其余操作。
> 2.已有缓存，发起get，lazy状态，不在refresh更新时间之内（数据失效）
> ![Alt text](./11-4.png)
> ![Alt text](./11-5.png)
> 蓝色箭头表示，数据过期，并发起一次新的请求，并缓存，执行操作。